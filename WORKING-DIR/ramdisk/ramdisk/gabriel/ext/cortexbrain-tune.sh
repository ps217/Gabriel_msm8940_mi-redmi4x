...
CPUBW_CLOCK_FREQ_SCALE()
{
	local state="$1";

	if [ "$state" == "awake" ]; then
		$BB sh /res/uci.sh devfreq_soc_cpubw "$devfreq_soc_cpubw" > /dev/null;
	elif [ "$state" == "sleep" ]; then
		$BB sh /res/uci.sh devfreq_soc_cpubw_susp "$devfreq_soc_cpubw_susp" > /dev/null;
	fi;
